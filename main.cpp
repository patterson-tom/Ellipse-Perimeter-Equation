...
double derivative(std::map<long double, long double> *ellipse_lengths, std::vector<long double> *w, int j) {
    ...
    for (std::map<long double, long double>::iterator it = ellipse_lengths->begin(); it != ellipse_lengths->end(); it++) {
        ...
        switch (j) {
            case 0:
                sum += -a * (it->second - hypothesis_function(a, b, w));
                break;
            case 1:
                sum += -b * (it->second - hypothesis_function(a, b, w));
                break;
            case 2:
                sum += ((0.05 * a * a) / (sqrt(w->at(2) * a * a + w->at(3) * a * b + w->at(4) * b * b))) * (it->second - hypothesis_function(a, b, w));
                break;
            case 3:
                sum += ((0.5 * a * b) / (sqrt(w->at(2) * a * a + w->at(3) * a * b + w->at(4) * b * b))) * (it->second - hypothesis_function(a, b, w));
                break;
            case 4:
                sum += ((0.5 * b * b) / (sqrt(w->at(2) * a * a + w->at(3) * a * b + w->at(4) * b * b))) * (it->second - hypothesis_function(a, b, w));
                break;
        }
    }
    return sum / ellipse_lengths->size();
...
void gradient_descent(std::map<long double, long double> *ellipse_lengths, std::vector<long double> *weights, int iterations = 1000, double learning_rate = 0.01, double learning_rate_modifier = 1) {
